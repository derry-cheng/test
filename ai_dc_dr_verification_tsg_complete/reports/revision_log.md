...
The independent event replay uses a predeclared baseline/event tariff pair of
85/210 USD/MWh, distinct from the gate contract's 120/150 USD/MWh pair, and a
predeclared 7-MWh response floor. The floor forces a measurable
...
for the five upstream artifacts. The certificate stores explicit profile roles
and deliberately does not assert that aggregate risk, payment, and job-indexed
profiles are numerically identical.
