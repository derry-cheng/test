...
`PYTHONPATH=src:vendor python tests/run_tests.py`; it covers workload-flow
feasibility, destination masks, SCED balance and line limits, N--1 enumeration,
endpoint certificates, causal information boundaries, provenance joins, budget
balance, and stage-manifest isolation.
...
| The verifier controls security-aware payment, not only credited MWh | Two lexicographically ordered daily LPs choose over six independent first-stage feasible projections plus the matched feasible-quantile comparator, embed every preventive N-1 dispatch row for held-out 10th/50th/90th-percentile power-conversion scenarios, and certify payment no larger than the same-scenario validation-selected single-projection cap; two additional interior conversion factors are replayed after selection by an independent 40-segment evaluator | `exp9_payment_certificate` |
...
| Aggregate counterfactual flow is tied to task-level evidence | Experiment 14 retains all 71,128 jobs for an observed contiguous witness; Experiment 19 solves a separate exact job-indexed counterfactual using submit-time Slurm service windows and a precommitted 0.001 MW/GPU cap, with observed completion used only for the native replay | `exp14_job_level_fidelity`, `exp19_job_level_counterfactual` |
...
| A deployment-time verifier cannot use future arrivals that are unavailable at the decision gate | The same exact workload LP and a refit causal metadata target mask the current-day ledger after the pre-event slot-48 gate; submitted committed-ledger baseline and post-event DR replay are solved separately, while the reserve remains planning-only | `exp17_decision_time_information` |
| AC network transfer is auditable under a shared active plan | Every declared native-case admissible outage on RTS-24/30/39/118 reuses the intact AC-OPF non-reference active outputs at three validation-only penetrations, with a pre-registered generator-bus electrical-role mapping and hard apparent-power/voltage limits | `exp18_preventive_ac_network_panel` |
| Job-level counterfactuals use only information available at submission | All 71,128 positive-energy jobs receive exact-energy service variables inside submit-time Slurm timelimit windows, with a bounded unlimited sentinel and a 0.001-MW/GPU nameplate; observed completion intervals are used only for native replay scoring | `exp19_job_level_counterfactual` |
...
Each experiment has a documented README, a shared implementation under `src/aicdr`,
and `results/intermediate`, `results/final`, and `figures` directories when its
stage is run. Figures are emitted as journal-quality English PNG and vector
