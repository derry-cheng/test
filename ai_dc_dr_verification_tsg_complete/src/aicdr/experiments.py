...
def run_exp2(root: Path, cfg: dict[str, Any], logger: logging.Logger) -> None:
    ...
    literature_specs = [
        (
            ...
            "strategic_reference",
            model_strategic,
            "structural analogue: exact workload LP with event reward; the cited paper's mechanism is not reimplemented",
        ),
        (
            ...
            model_honest,
            "structural analogue: exact ledger schedule without statistical projection; the cited four-step estimator is not reimplemented",
        ),
        (
            ...
            None,
            "structural analogue: validation-selected single ledger-feasible projection; rolling-horizon implementation is evaluated separately",
        ),
        (
            ...
            "event_response",
            None,
            "exact event-response LP with all four declared sites participating; the cited aggregator algorithm is not reimplemented",
        ),
    ]
...
def run_exp17(
    ...
    terminal equality for work that is not yet due.  A rolling committed-service
    solve then projects the gate-causal target onto the feasible set generated
    by the committed ledger; it does not suppress all flexible service.  The same
    locked actual trace is used only for scoring, so the result quantifies the
    deployment information cost without silently using future arrivals.
    """
    ...
    for local_day, day_value in enumerate(days):
        ...
        # removed from ``truncated``.  We therefore use the committed-ledger
        # physical capacity as the operational envelope and project the
        # gate-causal target onto the feasible set generated by ``truncated``.
        # This preserves a nonzero eligible service trajectory while keeping
        # every paid joule backed by a record available at the gate.  The
        # reference schedule remains an eligibility comparator, not a
        # zero-response cap.
        ...
        for method, profile, future_used, certificate_scope in [
            ...
            (
                ...
                False,
                "committed-ledger feasibility envelope",
            ),
            (
