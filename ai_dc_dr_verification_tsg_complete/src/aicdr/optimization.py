...
def solve_workload_schedule(
    ...
    if allowed_destinations is not None:
        allowed = np.asarray(allowed_destinations, dtype=bool)
        if allowed.shape != (sources, destinations):
            raise ValueError(
                "allowed_destinations has shape "
                f"{allowed.shape}, expected {(sources, destinations)}"
            )
    else:
        allowed = np.ones((sources, destinations), dtype=bool)
    ...
    # A structural baseline can restrict a source to a declared destination
    # subset without changing the workload conservation or deadline model.
    # Disallowed service variables are fixed at zero in the LP itself; this is
    # stronger than zeroing a returned profile after the solve and keeps the
    # comparator physically feasible.
    bounds = []
    for s in range(sources):
        for k in range(classes):
            for d in range(destinations):
                for _ in range(t_count):
                    bounds.append((0.0, None) if allowed[s, d] else (0.0, 0.0))
    bounds.extend([(0.0, None)] * n_y)
    if use_projection:
        bounds.extend([(0.0, None)] * n_z)
    result = linprog(
        c,
        A_ub=aub,
        b_ub=np.asarray(b_ub),
