...
Across 54 locked days containing 5,188,507 BurstGPT requests and 71,128
positive-energy jobs from the Massachusetts Institute of Technology (MIT)
SuperCloud scheduler--Data Center GPU Manager (DCGM) join, the risk-constrained
verifier reaches normalized root-mean-square error (nRMSE) 0.343382 and 1.716936 MWh/day of unsupported gross
credit; the independent tail-risk ensemble reaches 0.330795 and 2.523614
MWh/day. A separate job-indexed linear program optimizes 1,974,690 admissible
job--slot variables and reduces event energy by 0.058250 MWh while keeping the
maximum job-energy residual below $10^{-16}$ MWh. The locked N--1 panel covers
37 RTS-24 outages: trace-anchored exact and linear errors are $\$0.990$ and
$\$2.208$/day, whereas independent workload errors are $\$150.955$ and
$\$150.461$/day. Gate-causal response selection is calibrated only on the
16 earlier validation days and evaluated on the 54 locked days. Held-out
conversion endpoints, 1,704 fixed-active-plan alternating-current (AC) OPF
cells across four public networks, all 24 spatial permutations plus two
concentration controls, and a continuous
workload-hull interval audit define the reproducibility boundary; the
non-co-located traces are a benchmark rather than a utility field event.
\end{abstract}
...
security-constrained coordination, while aggregator coordination and
multi-level dispatch refine delay-sensitive electrical response
\cite{cao2022flexibility}.
...
job, \(r_j\) and \(d_j\) denote its submitted release and scheduler deadline,
\(E_j\) its measured energy, \(s_j\) its native region, and \(g_j\) its measured
GPU count. The indexed service variable \(u_{j\tau}\) is constrained by
\begin{equation}
\begin{aligned}
 &u_{j\tau}\geq0,\qquad
   \sum_{\tau=r_j}^{d_j-1}u_{j\tau}=E_j,\\
 &u_{j\tau}\leq\bar p_j\dt,\qquad
...
\label{eq:jobindexed}
\end{equation}
Here \(\bar p_j=E_j/((t_j^{\rm end}-t_j^{\rm start})/3600)\) and
\(\bar p_{\rm GPU}\) is the maximum observed average power per GPU in the
committed join. Thus the counterfactual is a globally solved, job-indexed
preemptive batch schedule with explicit release, deadline, energy, site
capacity, runtime-power, and GPU-count constraints. It is not obtained by
replaying the measured profile; its event tariff is fixed before scoring.
Experiment~19 solves (\ref{eq:jobindexed}) for all 71,128 positive-energy jobs.
...
permutations, three penetrations, and two concentration controls. Held-out
...
penalties and four risk reserves are validation-only. Ten-segment payments are
scored by an independent 40- or 80-segment evaluator. Moving three-day bootstrap
...
3\%/6\%/9\%. Experiment 11 exhausts 24 permutations plus two concentration
controls, three penetrations, four counterfactuals, and 54 days (16,848
outcomes): RiskSafe/single MAE is
...
Raw/scaled peaks are 0.008637/132.571 MW and $q_{01}/q_{99\text{-cap}}$ widths
are \$11.229/\$22.054 per day. With post-gate arrivals masked, the frozen
contract profile has nRMSE 0.451674 and gross false credit 1.238031 MWh/day.
The committed rolling response reaches nRMSE 0.441633, gross false credit
0.129469 MWh/day, and observable meter-capped credit 0.507916 MWh/day; its
credit recall is 0.019761 because only the committed ledger is eligible at the
gate. The complete-ledger RiskSafe comparator is 0.343382/1.716936 MWh/day
with recall 0.352763. The selected response price is \$150/MWh and projection
weight is 1.0, chosen on the 16 validation days under a 0.25-MWh false-credit
budget; the reserve quantile $\eta=0.60$ remains planning-only.
Table~\ref{tab:decision} separates payable credit from the offline meter
diagnostics. Experiment 18 covers 1,704 fixed-active-plan AC-OPF cells under the
pre-registered map; all converge (maximum loading 0.919291 p.u., zero voltage
violation, and zero non-reference active-plan deviation), so this remains a
steady-state model-boundary certificate.
...
\midrule
Gate-causal contract profile & 0.452 & 1.238 & 0 & 22.288 & 0.033\\
\textbf{Committed rolling response} & \textbf{0.442} & \textbf{0.129} & \textbf{0.508} & \textbf{21.871} & \textbf{0.020}\\
...
Experiment 19 removes the aggregate-only loophole by assigning a service
variable to every positive-energy job and every admissible 15-min slot between
its scheduler submission and observed completion deadline. Each job keeps its
native region, exact energy equality, GPU-count-derived power upper bound, and
the declared 118-MW regional capacity. A single sparse HiGHS program solves
the resulting preemptive batch-flow counterfactual; the measured contiguous
interval replay in Experiment 14 remains the nonpreemptive witness. Table~\ref{tab:jobcounterfactual}
reports the certificate. The solver reduces event-window energy by 0.058250
MWh while preserving every job's energy to $7.6\times10^{-17}$ MWh and leaving
at least 29.495 MWh of regional slot capacity. This establishes indexed
ledger feasibility without claiming that an unobserved counterfactual is
nonpreemptive or migratable.
...
Job--slot service variables & 1,974,690\\
Native event energy (MWh) & 0.194426\\
Counterfactual event energy (MWh) & 0.141231\\
Event reduction (MWh) & \textbf{0.058250}\\
Maximum job-energy residual (MWh) & $7.6\times10^{-17}$\\
Minimum regional-slot capacity slack (MWh) & 29.494982\\
\bottomrule
\end{tabular}
\end{table}
...
settlement. The final safe profile has nRMSE/false credit 0.343382/1.716936
MWh/day; tail-risk and feasible-quantile profiles reach 0.330795/2.523614 and
0.321370/2.917471. Trace-anchored N--1 exact/linear error is
\$0.990/\$2.208 per day, versus independent end-to-end
\$167.367/\$150.461/\$150.955. At the gate, the committed response yields
the calibrated observable contract result reported in Table~\ref{tab:decision};
the indexed counterfactual preserves all 71,128 job energies with an event
reduction of 0.058250 MWh.
