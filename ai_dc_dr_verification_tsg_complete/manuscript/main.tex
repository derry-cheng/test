...
unsupported credited response falls from 2.026218 to 1.910590 MWh/day (5.71\%)
and normalized root-mean-square error falls from 0.353737 to 0.348425 (1.50\%)
over 54 locked days. Exact N--1 settlement reduces trace-anchored error from
\$192.69 to \$0.90/day across 37 RTS-24 outages, while end-to-end errors are
\$207.67, \$155.25, and \$155.40/day for base-case exact, N--1 linear, and N--1
exact settlement. Independent replay covers every locked 15-minute slot and
...
$f_n\geq0$, a CVaR threshold $\nu\geq0$, and tail slacks $u_j\geq0$:
 \begin{align}
  ...
  &\leq\beta\,\mathrm{CVaR}_{0.75}(B_j^{\rm ref}).
...
the gate are removed before optimization and terminal completion is required
only for work due by the gate. We additionally solve a committed-ledger safe
...
cap.
 ...
 The finite cap is complemented by a set-valued payment certificate. For a
 realized counterfactual value, define
 $\Delta V_\xi(p)=V_{N-1}(p,\xi)-V_{N-1}(p^{1,\xi})$ and let
 $\mathcal P_{\rm seg}=\{p^{\rm ref},p^{\rm cert}\}$ denote the two
 validation-frozen endpoints of the contractual segment. The induced interval is
 \begin{equation}
  \mathcal I_\xi=\left[
  \min_{p\in\mathcal P_{\rm seg}}\Delta V_\xi(p),\;
  \max_{p\in\mathcal P_{\rm seg}}\Delta V_\xi(p)\right].
  \label{eq:paymentinterval}
 \end{equation}
 The selected reference payment is an element of $\mathcal I_\xi$ by
 construction. The interval endpoints use only these two feasible segment
 endpoints and the realized counterfactual needed for
settlement; no oracle baseline is used to select or certify the interval. Thus
(\ref{eq:paymentinterval}) makes uncertainty in the workload-to-power map
visible to the contract instead of relabeling a single forecast as true value.
It is not a lower-error claim: interval width is a reported uncertainty cost;
the larger seven-profile hull remains the finite-scenario payment certificate
in (\ref{eq:paymentcert}).
...
conversion factor and realized counterfactual, the endpoints in
(\ref{eq:paymentinterval}) are the exact minimum and maximum payments over the
declared two-endpoint feasible segment. This follows directly from the finite-set
minimum and maximum; it is an exact uncertainty statement for that contractual
segment, not an assertion that the segment contains every physically possible
schedule. Experiment~15 reports endpoint width for both held-out conversion
endpoints; no oracle baseline is used to select the segment.
...
MIT scheduler & 287,069 rows; submission time, duration, GPU count, and queue state\\
...
0.5845--2.6931 (1st--99th percentiles); Experiments 9, 11, and 15 evaluate this
interval rather than treating the 118-MW scale as a measured geographic fact.
...
credit falls from 2.026218 to 1.910590 MWh/day, a 5.71\% reduction (paired
three-day moving-block 95\% interval [0.057200, 0.534363] MWh;
Holm-adjusted $p=0.015625$), while nRMSE falls from 0.353737 to 0.348425
...
final total false-credit exposure is 412.687 MWh-slot versus 422.677 for the
single-projection reference, and its 75\% daily-tail exposure is 26.051 versus
26.617 MWh-slot; both aggregate certificates are satisfied. The two-sided
...
exposure is 1.910590 MWh/day and the mean under-credit is 15.313968 MWh/day,
compared with 15.244514 MWh/day for the selected single reference; every upper
...
\$1.92/day; ten Holm-adjusted block tests below 0.05), but improves only five
end-to-end workload cells (across-cell mean \$0.016/day). This separation
supports Proposition 3 without converting a trace diagnostic into an
end-to-end dominance claim.
...
signed-linear-minus-exact differences are $-0.01695$, $-0.02730$, $-0.01938$,
and $-0.01740$ USD/day, all below $0.03$ in magnitude, confirming local
affinity rather than a discretization artifact.
...
deadline-binding shares reach 0.0411 and 0.9118; mean false-credit exposure
across capacity levels is 2.641, 2.225, 1.971, 1.832, and 1.941 MWh. Every
schedule satisfies the declared workload constraints.
...
$2.3\times10^{-13}$ dollars; workload-verifier error is $107.28$ while its
budget residual is $1.6\times10^{-13}$. The exchangeable scaling panel reaches
20 participants (1,296 states; residual $2.98\times10^{-13}$), confirming exact
budget balance rather than permutation sampling.
...
base-case exact, N--1 linear, and N--1 exact are \$192.69, \$2.30, and \$0.90
per day; the last improves on base-case exact by \$191.79/day
(Holm-adjusted $p=3.1\times10^{-5}$), whereas its \$1.40/day advantage over
N--1 linear is not significant ($p=0.122$). Workload-verifier errors are
\$207.67, \$155.25, and \$155.40 (linear-to-exact $p=0.690$), showing that
counterfactual error dominates value linearization. Maximum post-contingency
...
anchored to the validation-selected single projection, with \RiskSafe\ external.
The held-out factors 0.734, 0.996, and 1.278 form $\Xi_3$; all 162 certificates
have zero positive cap violation and mean margins \$0.596, \$0.809, and
\$1.038/day. A 384-cell validation-only N--1 panel selects $\rho=1$ without
locked-test payment or peak outcomes. Independent 40-segment errors are
\$156.370/day for \PaymentSafe, \$155.605 for the selected single projection,
\$155.703 for feasible quantile, and \$155.562 for \RiskSafe. These scores do
not replace the deterministic cap.
...
through Proposition 5. Its two-endpoint feasible payment segment has mean
width \$0.545543/day at $q_{01}$ and \$4.065188/day at $q_{99}$ (maximum widths
\$14.483473 and \$128.381395/day). This interval is an explicit uncertainty
cost, not a lower-error claim. The cap is therefore a contractual upper bound
under the declared market model; it is not evidence that the payment-certified
...
all 864 base outcomes converge with zero voltage violation and at most 1.000-p.u.
loading; the payment guarantee remains scoped to (\ref{eq:paymentcert}). The
corrective panel adds 5,400 OPFs over IEEE-9/14 non-islanding outages. A shared
active plan is then fixed across all six IEEE-9 outages as in
(\ref{eq:acplan}); its 3,888 cells converge with zero plan deviation and
maximum loadings 0.876/0.899/0.923 p.u.
...
commitment; the released sensitivity file reports the capacity-safe clipped
factor (about 1.0005 at those endpoints) separately from the validation-frozen
...
gives slot MAE 4.287 versus 6.615 MW for the batch-only trace and 100\% slot
coverage; this is observational fidelity, not a causal event. A sparse flow LP
retains all 71,128 positive-energy jobs (1,974,690 service variables),
reconstructs 2.657346 MWh, and has residuals below $10^{-17}$ MWh. The observed
regional peak is 117.936 MW against the precommitted 118-MW nameplate. These
results establish ledger fidelity, not semantic-field authenticity.
...
cost over the declared closed interval. The two-endpoint feasible payment
segment has mean widths of \$0.545543/day and \$4.065188/day at these endpoints,
respectively. The decision-time audit removes all post-gate arrivals: the
truncated-ledger diagnostic changes nRMSE from 0.348425 to 0.525448 and credit
recall from 0.389559 to 0.041674, whereas the committed-ledger pointwise-safe
mode has zero false-credit exposure and a zero event-credit recall by design
because it does not credit uncommitted future work. Finally, 1,698 declared AC
...
exposure falls from 2.026218 to 1.910590 MWh/day relative to the matched
feasible-quantile reference, while nRMSE falls from 0.353737 to 0.348425. The
same comparison increases under-credit from 15.225 to 15.314 MWh/day and does
not improve $F_1$, so the empirical result is a risk tradeoff rather than
uniform predictive dominance. The trace-anchored mechanism-isolation panel
reduces N--1 exact-settlement error from \$192.69 to \$0.90/day, whereas the
end-to-end workload-verifier errors remain \$207.67, \$155.25, and \$155.40/day
for the three settlement variants. The job-level witness preserves 71,128
...
precommitted 118-MW nameplate with the observed regional envelope. The
