...
\usepackage{xcolor}
\usepackage{url}
\AtBeginEnvironment{thebibliography}{\fontsize{6.5pt}{6.8pt}\selectfont\setlength{\itemsep}{0pt}\interlinepenalty=0\clubpenalty=0\widowpenalty=0}
...
non-islanding IEEE Reliability Test System (RTS)-24 outages. An independent
real-execution replay covers every locked 15-minute slot, and a global
job-level flow certificate exactly represents all 71,128 positive-energy
...
and exact Shapley allocation provide separate evidence tiers for the claimed
verification-to-payment chain.
\end{abstract}
...
Recent power-system studies further coordinate geo-distributed clusters with
network operation \cite{zhang2024aggregator}.
...
This paper makes three power-system contributions. 1) We formulate an exact
workload-state demand model in which release, deadline, conservation,
cross-midnight completion, capacity, and migration constraints are enforced
simultaneously. A provenance-bound ledger certificate links each joined job to
its immutable scheduler/DCGM record, and a global job-level flow witness
preserves the measured execution energy of every joined job. 2) We construct a
validation-only, continuous-arrival counterfactual verifier with a pointwise
credit envelope, so the reported response is a feasible spatial--temporal
load trajectory rather than a meter-only prediction. 3) We develop signed
space--time nodal remuneration and a bilateral net-value contract under
complete preventive N--1 dispatch, and extend the payment certificate from
three finite telemetry quantiles to a held-out conversion interval by an
endpoint theorem. The contributions form one verification-to-payment chain:
the ledger certifies an attainable demand state, the envelope certifies
creditable response, and the security-aware value rule determines its
network value.
...
independent Data Center GPU Manager energy for 68,664 joined jobs
...
false-credit ratio of 0.344. Fig.~\ref{fig:manipulation} shows that the
profitable region follows the derived marginal boundary rather than a fitted
classifier.

\begin{figure*}[t]
\centering
\includegraphics[width=0.47\textwidth]{../experiments/exp1_manipulation/figures/fig1_manipulation_phase_diagram.pdf}
\caption{Experiment 1: exact strategic scheduling across the complete price and
call-probability grid. Markers show the independently calculated dual boundary.}
...
\TailRisk\ improves mean credit $F_1$ from 0.485 to 0.511 over the selected
...
reference; every upper and lower pointwise margin is nonnegative. Thus the
selector cannot obtain its false-credit result by collapsing the credited
trajectory below the reference, and the finite under-credit difference is
...
\begin{tabular}{lrrrr}
\toprule
Method & nRMSE & False ratio & False MWh & $F_1$\\
\midrule
High-5-of-10 & 2.653 & 0.856 & 118.820 & 0.230\\
Ridge & 1.845 & 0.788 & 90.582 & 0.321\\
Gradient boosting & 2.192 & 0.834 & 103.378 & 0.256\\
Extra Trees & 2.173 & 0.828 & 100.920 & 0.263\\
Online metadata & 2.123 & 0.824 & 98.681 & 0.275\\
Post-event metadata & 1.968 & 0.808 & 91.107 & 0.299\\
Post-event quantile & 1.891 & 0.782 & 87.223 & 0.324\\
Synthetic control & 1.893 & 0.819 & 88.185 & 0.270\\
Feasible quantile & 0.354 & 0.126 & 2.026 & 0.486\\
Single feasible & \textbf{0.350} & \textbf{0.122} & 1.957 & 0.485\\
Tail-risk feasible & 0.353 & 0.191 & 3.956 & \textbf{0.511}\\
Risk-constrained safe & \textbf{0.348} & \textbf{0.117} & \textbf{1.911} & 0.483\\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.46\textwidth]{../experiments/exp2_baseline_verification/figures/fig4_tuning_and_ablation.pdf}
\caption{Experiment 2: validation-fitted projection weights, constraint-set
ablation, specification robustness, and the precision--recall frontier. Every
point is produced by a globally solved workload program.}
...
offset---and selects day 69. Fig.~\ref{fig:case} shows source curtailment,
remote rebound, nodal prices, and line utilization in the same intervals. The
case explains why positive-only local accounting fails: migration creates a
simultaneous negative component that gross payment discards.

\begin{figure*}[t]
\centering
\includegraphics[width=0.43\textwidth]{../experiments/exp4_case_study/figures/fig7_spatial_response_case.pdf}
\caption{Experiment 4: predeclared spatial case. Source reduction and
destination rebound are evaluated jointly by signed nodal value.}
\label{fig:case}
\end{figure*}
...
rather than a favorable discretization. Fig.~\ref{fig:network} reports the
end-to-end cell effects, mechanism-isolation errors, and resolution
convergence separately.

\begin{figure*}[t]
\centering
\includegraphics[width=0.53\textwidth]{../experiments/exp5_network_robustness/figures/fig9_cross_network_robustness.pdf}
\caption{Experiment 5: native-rating cross-network results. (a) End-to-end
signed-linear-minus-exact mean absolute-error differences with the workload
verifier. (b) Trace-anchored mechanism-isolation error under matched prices.
(c) Full 54-day convergence at the near-affine RTS-24, 0.98-load boundary
cell; bars are 95\% standard-error intervals.}
\label{fig:network}
\end{figure*}
...
residual $2.98\times10^{-13}$ dollars. Fig.~\ref{fig:stress-allocation}
summarizes both the constraint-stress response and the exact allocation scaling.

\begin{figure*}[t]
\centering
\includegraphics[width=0.46\textwidth]{../experiments/exp6_physical_stress/figures/fig10_binding_constraint_stress.pdf}
\hfill
\includegraphics[width=0.46\textwidth]{../experiments/exp7_value_allocation/figures/fig11_exact_value_allocation.pdf}
\caption{Constraint sensitivity and exact allocation. (a) Complete capacity--
deadline stress grid. (b) Four-participant exact Shapley allocation and budget
residuals. The eight- and 20-participant scaling certificates are reported
numerically in the text.}
\label{fig:stress-allocation}
\end{figure*}
...
scored by a separate 40-segment N--1 dispatch. Fig.~\ref{fig:n1} compares
base-case exact, N--1 linear, and N--1 exact value. With the trace-anchored
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.52\textwidth]{../experiments/exp8_n1_security/figures/fig13_n1_security_validation.pdf}
\caption{Experiment 8: complete N--1 payment error, mechanism-isolation error
distributions, and loading under base-case versus N--1-secure dispatch.}
\label{fig:n1}
\end{figure*}
...
maximum post-contingency loading remains 1.000 p.u. Experiment 15 separately
audits the closed held-out conversion interval and is interpreted through
Proposition 5.
...
the declared shared-active-plan claim without contingency screening. Fig.~\ref{fig:ac-validation}
shows the intact, corrective, and shared-plan panels on a common scale.

\begin{figure*}[t]
\centering
\includegraphics[width=0.31\textwidth]{../experiments/exp10_ac_validation/figures/fig15_ac_opf_validation.pdf}
\hfill
\includegraphics[width=0.31\textwidth]{../experiments/exp10_ac_validation/figures/fig15b_ac_n1_contingency_validation.pdf}
\hfill
\includegraphics[width=0.31\textwidth]{../experiments/exp10_ac_validation/figures/fig15c_preventive_ac_n1_validation.pdf}
\caption{Nonlinear AC validation panels. (a) Intact AC optimal-power-flow
feasibility across public network sizes. (b) Corrective AC N--1 results for
all declared non-islanding outages. (c) Shared-active-plan preventive AC N--1
results with only reactive and loss-balancing recourse.}
\label{fig:ac-validation}
\end{figure*}
...
isolation result. Fig.~\ref{fig:spatial-scale} displays the complete
assignment--penetration panel.

\begin{figure*}[t]
\centering
\includegraphics[width=0.50\textwidth]{../experiments/exp11_spatial_scale_robustness/figures/fig16_spatial_scale_robustness.pdf}
\caption{Complete spatial-scale transfer. All 4! region-to-bus assignments,
three peak penetrations, four counterfactual methods, and 54 locked days are
shown without placement screening; the panels report error, loading, and
locational price spread.}
\label{fig:spatial-scale}
\end{figure*}
...
\subsection{Observed execution replay and job-level feasibility}

Experiment 13 addresses the first evidence boundary directly. It does not
intervene in the MIT workload or label an observational trace as a utility
event. Instead, every locked test day is replayed at 15-minute resolution from
the immutable scheduler/DCGM execution intervals. A batch-only minimum-cost
schedule is compared with an exact batch-only schedule constrained to the
measured aggregate trace. The trace-constrained replay reduces mean slot MAE
from 6.615 to 4.287 MW and mean absolute energy discrepancy from 635.058 to
411.516 MWh over all 54 days, with 100\% slot coverage. These numbers verify
that the same release/deadline ledger can reproduce the observed execution
trace; they are not a causal estimate of a counterfactual event response.
Table~\ref{tab:replay-flow} reports the paired replay errors for every locked
day.

Experiment 14 removes divisible aggregate-flow ambiguity by retaining every
positive-energy joined job and solving one sparse release/deadline flow LP over
the complete observed horizon. The formulation contains 71,128 jobs and
1,974,690 service variables; it reconstructs 2.657346 MWh of input energy with
maximum job-completion and aggregate-slot residuals of
$6.0\times10^{-18}$ and $9.4\times10^{-18}$ MWh, respectively. The empirical
telemetry envelope is compared post hoc with the declared 118-MW flexible
nameplate; the maximum observed regional batch peak is 117.936 MW, giving a
peak-to-nameplate ratio of 0.9995. This reconciliation tests whether the
precommitted nameplate covers the observed execution envelope without using
locked outcomes to choose it. Table~\ref{tab:replay-flow} gives the
corresponding job and slot residuals.
...
Flow witness & Positive-energy joined jobs & -- & 71,128 \\
Flow witness & Sparse service variables & -- & 1,974,690 \\
Flow witness & Target energy / max job residual & -- & 2.657346 MWh / $6.0\times10^{-18}$ MWh \\
Flow witness & Max aggregate residual / peak ratio & -- & $9.4\times10^{-18}$ MWh / 0.9995 \\
Interval certificate & Endpoint margin at $q_{01}$ & -- & \$0.545543/day; 54 days, 8 intervals \\
Interval certificate & Endpoint margin at $q_{99}$ & -- & \$4.065188/day; 37 non-islanding outages \\
Interval certificate & Maximum positive cap violation & -- & $1.7\times10^{-8}$ USD; 40-segment evaluator \\
Ledger provenance & Joined positive-energy jobs & -- & 71,128; 287,173 scheduler / 96,893 telemetry rows \\
Ledger provenance & Sparse service variables & -- & 1,974,690; exact release/deadline flow \\
Ledger provenance & Max job / aggregate residual & -- & $6.0\times10^{-18}$ / $9.4\times10^{-18}$ MWh \\
Ledger provenance & Peak / nameplate ratio & -- & 117.936 / 118 MW = 0.9995 \\
\bottomrule
\end{tabular}%
}
\end{table}

\subsection{Continuous telemetry-conversion interval audit}

Experiment 15 independently evaluates the locked \PaymentSafe{} profile
against the validation-selected single feasible reference at the two held-out
energy-conversion endpoints, $q_{01}=0.5845$ and $q_{99}=2.6931$. The
feasible-quantile profile remains an external transfer comparator. All 54 days, eight event
intervals, 37 non-islanding contingencies, and an independent 40-segment
generator-cost evaluator are retained. The payment-certified profile has
nonnegative endpoint margins up to the $1.7\times10^{-8}$-USD numerical
tolerance; the minimum margin is zero. The mean endpoint margin is
\$0.545543/day at $q_{01}$ and \$4.065188/day at $q_{99}$. By Proposition 5, these
endpoint results certify that the candidate's worst-case cost over the closed
conversion interval is no larger than the selected-single reference's worst-case
cost. They
do not replace the samplewise finite-scenario guarantee of Proposition 4 with
an unjustified pointwise claim for every interior factor. The endpoint margins,
cap residual and exact job-flow residuals are retained with the locked results.
Table~\ref{tab:replay-flow} reports the two endpoint margins and the
closed-interval certificate.

\subsection{Ledger provenance and capacity reconciliation}

Experiment 16 independently audits the same-source MIT scheduler/DCGM release.
The 287,173 scheduler rows and 96,893 telemetry rows yield 71,128 positive-
energy joined jobs after the declared last-record rule. Every retained row
satisfies release-before-start and start-before-end; the retained raw telemetry
energy and the joined ledger energy agree to machine precision. The canonical
sorted ledger digest is retained in the provenance certificate (prefix
\texttt{fdf49ad75d301580}); no synthetic rows or imputed deadlines are used.
Table~\ref{tab:replay-flow}
summarizes the integrity checks, independent energy conservation, and regional
capacity envelope. This certificate binds the job-level flow witness to the
exact public records used by the model; the public network transfer remains a
trace-driven benchmark because the workload and utility traces are not
co-located.

\subsection{Decision-time information boundary}

Experiment 17 fixes the information set at the first event slot (slot 64).
All arrivals after that gate are removed from the optimization input, while
the locked execution trace is used only for scoring. The resulting
decision-time verifier solves all 54 event-day linear programs with the same
release, deadline, migration, and capacity model. The mean post-gate arrival
energy is 209.794 MWh per day. Relative to the complete-ledger audit, the
decision-time profile has nRMSE 0.525448 versus 0.348412 and credit recall
0.041674 versus 0.389559. This panel measures the information cost of an
online deployment boundary; it does not use future arrivals to select a
decision-time profile. Fig.~\ref{fig:decision-time} reports the paired
 information-set and credit metrics for every locked day.

\begin{figure}[t]
\centering
\includegraphics[width=0.60\columnwidth]{../experiments/exp17_decision_time_information/figures/fig23_decision_time_information.pdf}
\caption{Decision-time information-boundary audit. The truncated-ledger
verifier uses no arrivals after the event gate; the complete-ledger profile is
an information-cost comparator scored on locked execution.}
\label{fig:decision-time}
\end{figure}

\subsection{Cross-network AC contingency audit}

Experiment 18 extends the nonlinear check to RTS-24, IEEE-30, IEEE-39, and
IEEE-118. For each public case, the admissible outage set is fixed before
workload outcomes by connected topology and a finite native-case AC power-flow
solution; every outage in that set is then evaluated at 3\%, 6\%, and 9\%
penetration for the payment-certified and trace-anchored profiles. The
fixed-day rule uses the first locked test day and the first event slot, while
the penetration scale is computed from validation traces only. The panel
 contains 1,698 method--penetration--outage rows and solves every declared
 admissible AC recourse problem. Fig.~\ref{fig:ac-cross-network} displays
apparent loading, voltage diagnostics, and active-generation displacement by
network. Because the recourse is a nonlinear AC power-flow audit rather than
a simultaneous preventive AC security-constrained OPF, these diagnostics
complement the exact preventive DC certificate in (\ref{eq:paymentcert}) and
the shared-active-plan IEEE-9 panel in Experiment 10; they do not silently
replace either model with a weaker feasibility claim.

\begin{figure}[t]
\centering
\includegraphics[width=0.60\columnwidth]{../experiments/exp18_preventive_ac_network_panel/figures/fig24_preventive_ac_cross_network.pdf}
\caption{Cross-network nonlinear AC contingency audit. Every declared native-
case admissible outage is evaluated at three validation-scaled penetrations;
the panels report apparent loading, voltage diagnostics, and active recourse.}
...
reference, while nRMSE falls from 0.353737 to 0.348412. The trace-anchored
mechanism-isolation panel reduces
N--1 exact-settlement error from \$192.69 to
\$0.90/day. The job-level witness preserves 71,128 measured jobs with
machine-precision residuals, and the provenance certificate binds the witness
to the immutable scheduler/DCGM join and reconciles the precommitted 118-MW
nameplate with the observed regional envelope. The decision-time panel
quantifies the information cost of removing post-gate arrivals, while the
cross-network AC audit evaluates every declared native-case admissible outage
on four public systems with validation-only power scaling.
The public traces provide reproducible workload and network evidence rather
than a utility field intervention. Deployment therefore requires a signed
operator ledger, co-located facility telemetry, topology access, and a
...
\begingroup
\fontsize{5.2pt}{5.4pt}\selectfont
