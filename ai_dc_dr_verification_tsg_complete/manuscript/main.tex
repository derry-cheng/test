...
0.343 with 1.717 MWh/day of unsupported gross credit. At a gate four
hours before the event, the committed-ledger operating replay has nRMSE
0.431, 0.048 MWh/day of gross false credit, and 0.330 MWh/day of
meter-capped payable credit; the submitted baseline is frozen before the event.
...
the committed 118-MW nameplate at a certified 3475.108$\times$ scale while
preserving every job-level equality and admissible window. The security panel evaluates all 37 finite IEEE
...
\begin{equation}
 ...
 [\widehat p^0_{dt}-\max\{p^{\rm obs}_{dt},p^{1,\mathrm{sim}}_{dt}\}]_+ .
...
power-transfer distribution factor (PTDF) $H$, and line ratings $\bar f$, the
...
$f_n\geq0$, a CVaR threshold $\nu\geq0$, and tail slacks $u_j\geq0$:
 \begin{align}
  f_n&\geq p^\alpha_n-\max\{p^{\mathrm{obs}}_n,p^{1,\mathrm{sim}}_n\},\\
  F_j&=\sum_{n\in j}f_n,\\
  F_j-\nu-u_j&\leq0,\\
 ...
 \end{align}
Here $p^{\mathrm{obs}}$ and $p^{1,\mathrm{sim}}$ in the validation ceiling are
labels for offline risk selection only. They are never substituted for the frozen contract profile
...
The independent trace-meter replay provides the complementary observational
check required for deployment: mean event-window MAE is 2.701 MW for the
feasible-quantile projection, 2.812 MW for \TailRisk, and 2.864 MW for
\RiskSafe over the same 54 locked days, with three-day block 95\% intervals
reported in Experiment~20. These values measure alignment with the closed
DCGM execution trace; they do not label the trace as a utility intervention or
replace the mechanism-isolation response panel. Figure~\ref{fig:trace-replay}
visualizes the independent replay intervals.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{../experiments/exp20_trace_meter_replay/figures/fig25_trace_meter_replay.pdf}
\caption{Independent locked-day trace-meter replay. Points show mean event-window
MAE against the measured execution tensor; whiskers are three-day moving-block
95\% intervals. The panel is observational and contains no utility-event label.}
\label{fig:trace-replay}
\end{figure}

The closest domain baselines are equation-level structural controls, not
software reimplementations. The spatial-DR, flexibility, receding-horizon, and
all-site exact event-response controls have daily (nRMSE, false credit, $F_1$) of
(3.063,114.572,0.137), (0.370,1.549,0.411), (0.345,1.849,0.442), and
(0.329,0,0.279). DC--T/DC--ST translations give (0.370,1.549) with
mean migration 0/4.328 MWh. All controls share the ledger, deadlines,
...
6 solves 810 capacity/deadline schedules, with binding shares 0.031--0.911
and capacity-level false-credit means 2.374--2.740 MWh.
...
15 gives interval widths \$11.229/\$22.054 per day at $q_{01}$ and the
capacity-safe $q_{99}$ endpoint (maxima \$50.090/\$97.951); the raw
$q_{99}=2.6931$ is clipped to 1.150. Four of 108 cases lie inside the
...
corrective IEEE-30/39/118 outages satisfy the limits, and 3,888 shared-active-plan
cells have zero active-plan deviation (maximum loadings
0.880/0.907/0.935 p.u. at 3\%/6\%/9\%). Experiment 11 evaluates all
...
complete-cycle MAE \$0.641/\$0.658/\$0/\$0 and event-window MAE
\$21.882/\$29.287/\$5.325/\$5.325; all 54 bilateral contracts
activate.

\subsection{Independent trace and deployment-boundary audits}

Immutable MIT scheduler/DCGM replay gives slot MAE 3.740 versus 5.678 MW
with 100\% coverage; the 71,128-job witness residual is below $10^{-17}$ MWh.
The independent trace-meter panel reports mean event-window MAE 2.864 MW for
\RiskSafe (2.701 MW for feasible quantile) with block intervals, and remains
strictly observational. At the slot-48 gate, the submitted no-tariff baseline
is frozen before scoring, whereas the committed-ledger operating replay reaches
nRMSE 0.431, gross false credit 0.048 MWh/day, meter-capped payable
credit 0.330 MWh/day, and recall 0.015. The complete-ledger RiskSafe comparator is
0.343/1.717 MWh/day with recall 0.353. The selected \$150/MWh and
...
offline meter diagnostics. Experiment 18 supplies a 9,900-cell fixed-active-
plan AC-OPF certificate over three predeclared locked days and two event-slot
endpoints under the pre-registered map (maximum loading 0.917 p.u.; zero
voltage and non-reference active-plan violations).
...
0.194/0.028 MWh, giving 0.183 MWh gross, 0.167 MWh net reduction,
and 0.016 MWh rebound; maximum job residual is
$2.17\times10^{-19}$ MWh and minimum capacity slack is 29.492 MWh. The
separate Exp.~21 audit certifies a capacity-safe 3475.108$\times$ homogeneous
scale at the 118-MW nameplate without using locked-test outcomes.
...
Native event energy (MWh) & 0.194\\
Counterfactual event energy (MWh) & 0.028\\
Gross event reduction (MWh) & \textbf{0.183}\\
Net event reduction (MWh) & \textbf{0.167}\\
...
settlement. The safe profile reaches nRMSE/false credit 0.343/1.717
...
job energies and yields 0.167 MWh net reduction, 0.183 MWh gross
reduction, and 0.016 MWh rebound. The scale-consistency certificate confirms
that this indexed primal reaches the 118-MW nameplate under a 3475.108$\times$
trace scale while retaining all ledger constraints. Deployment requires signed co-located
